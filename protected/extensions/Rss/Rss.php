...
class Rss
{
    ...
    public function parse()
    {
        if(($data = cache()->get(CacheNames::RSS)) === FALSE)
        {
            ...
            {
                throw new Exception(Yii::t('main', 'Не удалось подключиться к RSS, URL: :url', array(':url' => $this->_url)));
            }
    ...
    {
        return isset($item['description']) ? str_replace(array('<![CDATA[', ']]>'), '', $item['description']) : '';
    }
