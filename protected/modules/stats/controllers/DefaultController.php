...
class DefaultController extends FrontendBaseController
{
	...
	public function actionIndex($gs_id = NULL, $type = '', $clan_id = NULL)
	{
        ...
        if($gs_id !== NULL && isset($this->_gs_list[$gs_id]))
        {
            ...
            {
                $this->redirect(array('/stats/default/index', 'gs_id' => $gs_id, 'type' => $this->_stats_types[0]));
            }

            if($type != '' && !in_array($type, $this->_stats_types))
            {
                $content = Yii::t('main', 'Статистика недоступна.');
            }
            elseif($type != '' && in_array($type, $this->_stats_types))
            {
                ...
                {
                    $content = Yii::t('main', 'Метод для обработки статистики не найден.');
                }
            }
        }
    ...
    private function statsTypes()
    {
        ...
        {
            if(strpos($k, 'stats') !== FALSE && $v == 1 && !in_array($k, array('stats_allow', 'stats_cache_time', 'stats_count_results')))
            {
    ...
    public function clan_info()
    {
        ...
        $command->where('clan_data.clan_id = :clan_id', array(':clan_id' => $this->_clan_id));
        ...
        if($clanInfo)
        {
            ...
            $command->where($clanIdFieldName . ' = :clan_id', array(':clan_id' => $this->_clan_id));
