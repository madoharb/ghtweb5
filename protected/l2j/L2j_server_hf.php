...
class L2j_server_hf
{
    ...
    {
        $this->_db->createCommand()->insert('accounts', array(
            'login'        => $login,
            'password'     => $this->_context->passwordEncrypt($password),
            'accessLevel'  => $access_level,
        ));

        $this->_db->createCommand()->insert('account_premium', array(
            'account_name'    => $login,
            'premium_service' => 0,
            'enddate'         => 0,
        ));
    ...
    {
        $res = $this->_db->createCommand("SELECT * FROM {{account_premium}} WHERE account_name = :account_name AND premium_service = 1 LIMIT 1")
            ->bindParam('account_name', $accountName, PDO::PARAM_STR)
            ->queryRow();

        return array(
            'dateEnd' => (isset($res['enddate']) && is_numeric($res['enddate']) ? substr($res['enddate'], 0, 10) : 0),
        );
    }
    ...
    {
        return $this->_db->createCommand("UPDATE {{account_premium}} SET enddate = :enddate, premium_service = 1 WHERE account_name = :account_name")
            ->bindParam('enddate', $timeEnd, PDO::PARAM_INT)
            ->bindParam('account_name', $accountName, PDO::PARAM_STR)
            ->execute();
    }
